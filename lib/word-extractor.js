...
class WordExtractor {
    ...
    extract(text) {
        const sentences = text.split(RE_SENTENCE_DELIMETER)
            .filter(sentence => !RE_EMPTY_LINE.test(sentence));
        const generalWords = _.flatten(sentences.map(this._extractFromSentence.bind(this)));
        return _.uniq(generalWords);
    }

    _extractFromSentence(sentence) {
        const words = this._tokeniser.tokenise(sentence)
